...
export default function FilterGroup({
  ...
  const defaultValues = [
    filter.range.selectedMin,
    filter.range.selectedMax,
  ];
  const [open, setOpen] = useState(true);
  const [values, setValues] = useState(filter.selectionType === "range"
            ...
            {filter.values?.map((option) => {
              const checked =
                selectedFilters[filter.featureId]?.some(
                  (item) =>
                    item.valueCode === option.valueCode
                ) || false;
              const Icon =
                filterIcons[option.displayName];
              return (
                <button

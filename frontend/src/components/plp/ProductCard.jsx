...
export default function ProductCard({ product, filters }) {
  ...
  const metalFilter = filters?.find(
    (f) => f.featureId === "FEATURE-METAL"
  );

  const availableMetals =
    product.availableValues?.["FEATURE-METAL"] || [];

  const selectedMetal =
    product.defaultSelection?.["FEATURE-METAL"];
  const discount =
    Math.round(
      (
          ...
          <div className="flex items-center gap-3 mt-5">
            {availableMetals.map((valueCode) => {
              ...
              return (
                <div
                  key={valueCode}
                  className={`flex h-6 w-6 items-center justify-center rounded-full ${selected ? "border-2 border-[#9C6D4B]" : ""
                    }`}
                >
                  <span
